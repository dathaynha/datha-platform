 ...
 *
 * Standalone chatbot only checks angular-oauth2-oidc's localStorage view
 * (`hasValidAccessToken()` = access_token present + expires_at in the future);
 * signatures are never verified in the browser. The gateway DOES verify on API
 * calls, so the JWT carries the gateway's claim shape (sub = owner id; HS256) —
 * set E2E_JWT_SECRET to the gateway's secret when a spec wants live API
 * responses, otherwise any value works because specs route-mock the gateway.
 */
...
export default function globalSetup(): void {
  ...
  // Keys the standalone remote reads: angular-oauth2-oidc storage + LOCAL_STORAGE_KEY.
  const localStorage = [

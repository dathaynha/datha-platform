...
import { installDatepickerCompactStyles } from "src/styles/install-datepicker-compact";
...
export class AuthenticatedLayoutComponent implements OnInit {
  ...
  readonly eventsLink = signal<string[]>(["/events"]);
  readonly dlqLink = signal<string[]>(["/dlq"]);
  ...
  readonly tabs = computed<DathaSubHeaderTab[]>(() => [
    ...
    { id: "events", labelKey: "SUBNAV.EVENTS", route: this.eventsLink() },
    { id: "dlq", labelKey: "SUBNAV.DLQ", route: this.dlqLink() },
  ]);
  ...
  ngOnInit(): void {
    installDatepickerCompactStyles(document);

    const shelled = !!this.route.snapshot.data["shelled"];
    this.isShelled.set(shelled);
    if (shelled) {
      this.homeLink.set(["/event-store"]);
      this.eventsLink.set(["/event-store/events"]);
      this.dlqLink.set(["/event-store/dlq"]);
      registerRemoteTranslations(
        this.translate,
        this.translateStore,

...
/**
 * Envelope parsing for the events this service consumes.
 *
 * A copy of notification-service's parser: there is no shared backend package
 * in this workspace, so each consumer carries its own — same reason every
 * service has its own `src/nats/streams.ts`.
 */
const UUID_RE =
...
/** Tolerates unknown extra fields — consumers must survive producer schema additions. */

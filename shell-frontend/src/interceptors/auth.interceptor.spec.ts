...
describe("AuthInterceptor", () => {
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: DialogService, useValue: { open: () => null } },
        {
          ...
          useValue: { instant: () => ({ title: "t", message: "m" }) },
        },
  ...
  describe("api bearer targeting", () => {
    ...
    it("attaches the bearer to gateway API calls", () => {
      const req = intercepted(
        "http://localhost:8080/api/notifications/stream-token",
      );
  ...
  /**
   * A federated remote cannot use the context token: it bundles its own copy of
   * the token file, so the object identity differs from the one this
   * interceptor reads, and the opt-out is silently ignored. Hosted by the
   * shell, this interceptor is the one that runs for a remote's requests —
   * which is how a background chat poll started raising the session dialog over
   * the whole app. The header carries the same intent as a value.
   */
  describe("quiet-errors header (crosses the federation boundary)", () => {
    function interceptOnce(headers: Record<string, string>) {
      ...
      const request = new HttpRequest(
        "GET",
        `${environment.gateway.baseUrl}/conversations`,
    ...
    it("strips the header before the request is sent", () => {
      ...
      // The marker is for this interceptor only; upstream must never see it.
